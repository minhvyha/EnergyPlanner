      ...
      <div>
        ...
        <div className="px-6">
          ...
          <div className="bg-white border-[3px] border-black rounded-[24px] p-5 mb-6 ">
            <p className="text-gray-400 text-sm font-medium mb-2">Current Energy Level</p>
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-3xl font-bold text-black">{energyLevel}</h3>
                  {energyLevel === 'LOW' && <BatteryLow className="text-[#FF5A7C]" size={28} />}
                  {energyLevel === 'MED' && <BatteryMedium className="text-[#FDE047]" size={28} />}
                  {energyLevel === 'HIGH' && <BatteryFull className="text-[#95E9C1]" size={28} />}
                </div>
                <p className="text-gray-400 text-sm">Take it easy today</p>
              </div>
              <button
                onClick={() => setShowEnergyModal(true)}
                className="flex items-center gap-2 text-black font-bold hover:opacity-70 transition-opacity"
              >
                <span className="text-sm">Tap to update</span>
          ...
          {/* Focus Section */}
          <div className="mb-6">
            <h3 className="text-lg font-bold text-black mb-2">Focus for this energy:</h3>
            <p className="text-gray-600">Light review and admin</p>
          </div>

          {/* Suggested Tasks */}
          <div className="mb-6">
            <h3 className="text-lg font-bold text-black mb-4">Suggested Tasks</h3>
            {suggestedTasks.length === 0 ? (
              <p className="text-gray-500 text-center py-8">
                No tasks available. Create tasks in the Task Library to see suggestions here.
              </p>
            ) : (
              <div className="space-y-4">
                {suggestedTasks.map((task) => (
                  ...
                  >
                    <h4 className="text-lg font-bold text-black mb-2">{task.title}</h4>
                    <p className="text-gray-500 text-sm mb-3">
                      {task.duration} {task.subject && `| ${task.subject}`}
                    </p>
                    <div className="flex items-center justify-between">
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 ${
                        task.energyLevel === 'low' ? 'bg-[#95E9C1]' :
                        task.energyLevel === 'med' ? 'bg-[#FDE047]' : 'bg-[#FF5A7C]'
                      } border-[2px] border-black rounded-full`}>
                        <svg width="12" height="14" viewBox="0 0 12 14" fill="none">
                          <path d="M6.5 1L1 8h5l-.5 5 5.5-7H6l.5-5z" fill="currentColor" className={
                            task.energyLevel === 'low' ? 'text-[#95E9C1]' :
                            task.energyLevel === 'med' ? 'text-[#FDE047]' : 'text-[#FF5A7C]'
                          } />
                        </svg>
